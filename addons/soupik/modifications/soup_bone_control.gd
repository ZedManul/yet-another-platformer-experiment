...
## Only has effect if Control Position is set to false; Makes itself or the target node inherit position from the bone.
@export var inherit_bone_rotation: bool
...
func handle_rotation_control(str: float) -> void:
	...
	if target_node: _target = target_node
	if !control_rotation: 
		if !inherit_bone_rotation: return
		_target.global_transform.x = bone_node.global_transform.x
		_target.global_transform.y = bone_node.global_transform.y
		return
